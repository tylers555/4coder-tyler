...
lister_render(Application_Links *app, Frame_Info frame_info, View_ID view){
    ...
    i32 last_index = Min(first_index+i32_ceil32(rect_height(list_rect)/block_height)+5,
                         count);
    ...
    for (i32 i = first_index; i < last_index; i += 1){
        ...
        Fancy_Line line = {};
        push_fancy_string(scratch, &line, node->color, node->display_name);
...
function void*
lister_add_item(Lister *lister, Lister_Prealloced_String string, Lister_Prealloced_String display_name, 
                Lister_Prealloced_String status, void *user_data, u64 extra_space, 
    ...
    node->display_name = display_name.string;
...
lister_add_item(Lister *lister, Lister_Prealloced_String string, Lister_Prealloced_String status, 
                void *user_data, u64  extra_space, FColor color=fcolor_id(defcolor_text_default)){
    return(lister_add_item(lister, string, string, status, user_data, extra_space, color));
}

function void*
lister_add_item(Lister *lister, Lister_Prealloced_String string, String_Const_u8 status, 
                void *user_data, u64  extra_space, FColor color=fcolor_id(defcolor_text_default)){
    return(lister_add_item(lister, string, lister_prealloced(push_string_copy(lister->arena, status)), user_data, extra_space, color));
}
...
lister_add_item(Lister *lister, String_Const_u8 string, Lister_Prealloced_String status, 
    ...
    return(lister_add_item(lister, lister_prealloced(push_string_copy(lister->arena, string)), status, 
                           user_data, extra_space, color));
}

function void*
lister_add_item(Lister *lister, String_Const_u8 string, String_Const_u8 status, 
                void *user_data, u64 extra_space, FColor color=fcolor_id(defcolor_text_default)){
...
function void*
lister_add_item(Lister *lister, String_Const_u8 string, String_Const_u8 display_name, String_Const_u8 status, 
                void *user_data, u64 extra_space, FColor color=fcolor_id(defcolor_text_default)){
    return(lister_add_item(lister,
                           lister_prealloced(push_string_copy(lister->arena, string)),
                           lister_prealloced(push_string_copy(lister->arena, display_name)),
                           lister_prealloced(push_string_copy(lister->arena, status)),
                           user_data, extra_space, color));
}
